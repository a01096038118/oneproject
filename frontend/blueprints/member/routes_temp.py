...
#  관리자 키 생성 페이지
@admin_bp.route('/key_page', methods = ['GET'])
def admin_key_page():
...
# 관리자 회원가입 화면 이동
@admin_bp.route('/signup_form', methods = ['GET'])
...
# 관리자 회원가입 양식
@admin_bp.route('/signup_confirm', methods = ['POST'])
...
# 관리자 로그인 화면 
@admin_bp.route('/signin_form', methods = ['GET'])
def adminSignIn_form():
    return render_template('member/adminSignIn_form.html')

# 관리자 로그인 양식
@admin_bp.route('/signin_confirm', methods = ['POST'])
def adminSignIn_confirm():
    ...
    for admin in admins:
        ...
        if admin['mId'] == mId:
            if admin['mPw'] != mPw:
                return render_template('admin/adminSignIn_result.html',
...
def memberModify_confirm():
    ...
    for member in members:
        if member['mId'] == mId:
            ...
            return render_template('member/memberModify_result.html',
