...
PROJECT_ROOT = os.path.abspath(os.path.join(CURRENT_DIR, "..", "..")) # first_project

# 🚨 [경로 변경] 조장님이 새로 만드신 db/json/ 폴더 밑으로 경로를 지정했습니다.
INCIDENT_LOG_DB_PATH = os.path.join(PROJECT_ROOT, "db", "json", "capture_logs.json")
EXCEL_REPORT_PATH = os.path.join(PROJECT_ROOT, "db", "json", "rescue_report.csv")

# json 폴더가 혹시 비어있더라도 실행 시 안전하게 자동 생성해 줍니다.
if not os.path.exists(os.path.dirname(INCIDENT_LOG_DB_PATH)):
    os.makedirs(os.path.dirname(INCIDENT_LOG_DB_PATH))


def _save_to_excel(new_incident):
    """[내부 함수] 엑셀 파일에 로그와 클릭 링크를 추가합니다."""
...
def save_logs(image_file):
    ...
    조장님이 만든 json 폴더 밑의 JSON 파일과 엑셀 리포트에 로그를 동시에 기록합니다.
    """
    # 1. 이미지 핸들러 함수(save_images) 호출
...
def get_logs():
    """
    [조회 엔진] 대시보드 화면용으로 
    지정한 json 폴더 내부의 모든 로그 리스트를 읽어서 반환합니다.
    """
